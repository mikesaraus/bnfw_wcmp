...
if ( ! class_exists( 'BNFW_Engine', false ) ) {
	...
	class BNFW_Engine {
		...
		public function get_emails_from_users( $users, $exclude = null, $post_id = 0, $setting = array() ) {
			$user_ids     = array();
			$user_roles   = array();
			$user_wc_mp   = array(); // WooCommerce Members
			$non_wp_users = array();
			...
			foreach ( $users as $user ) {
				// GM - ms
				// Filter if Selected WooCommerce Memebership Plan
				if ( $this->starts_with( $user, 'wc_mp_' ) ) {
					$user_wc_mp[] = str_replace( 'wc_mp_', '', $user );
				} elseif ( $this->starts_with( $user, 'role-' ) ) {
			...
			// GM - ms
			// Filtered Active User Email from WooCommerce Subscription Plan 
			$emails_from_wc_mp = $this->get_emails_from_membership_plan_id( $user_wc_mp, $exclude );
			...
			return array_merge( $emails_from_user_roles, $emails_from_user_ids, $emails_from_wc_mp, $non_wp_emails );
		}
		...
		/**
		 * GM - ms
		 * Check all Dependecies if Active
		 */
		...
		/**
		 * GM - ms
		 * Get emails of users based on woocommerce membership plans subcription.
		 *
		 * @param array $plans Membership Plans.
		 * @param int   $exclude User id to exclude.
		 * @return array Email ids
		 */
		private function get_emails_from_membership_plan_id( $plans, $exclude = null ) {
			if (! $this->check_all_dependencies()) {
				return array();
			}
			...
			foreach ( $plans as $plan ) {
				// Remove Prefix for Plan ID
				$plan = str_replace( 'wc_mp_', '', $plan );
				$plan = wc_memberships_get_membership_plan($plan);
				
				// All Users with Active Membership
				$users = $this->get_all_active_subscribers();
				...
				foreach ( $users as $user ) {
					...
					// Is User Active on this Membership Subscription Plan 
					if ( ! wc_memberships_is_user_active_member($user->ID, $plan->get_slug())) {
						continue;
					}
